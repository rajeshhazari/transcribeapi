...
import com.c3transcribe.transribeapi.api.model.dto.RegisterUserDto;
import com.c3transcribe.transribeapi.api.services.JwtUserDetailsService;
import com.c3transcribe.transribeapi.api.util.JwtUtil;
import com.c3transcribe.transribeapi.api.domian.AppUsers;
import com.c3transcribe.transribeapi.api.model.dto.AppError;
import com.c3transcribe.transribeapi.api.models.AuthenticationRequest;
import com.c3transcribe.transribeapi.api.models.AuthenticationResponse;
...
public class JwtAuthenticationController {
	...
	public ResponseEntity<?> createAuthenticationToken(@RequestBody AuthenticationRequest authenticationRequest) throws Exception {
		...
		try {
			...
			logger.debug("encode passwd:: {} for password {}: ", java.util.Base64.getDecoder().decode(authenticationRequest.getPassword().getBytes())  , authenticationRequest.getPassword());
			...
			Authentication auth = authenticationManager.authenticate(
					new UsernamePasswordAuthenticationToken(authenticationRequest.getUsername(), hashedPassword) );
	...
	public ResponseEntity<?> registerUser(@RequestBody RegisterUserDto user) throws Exception {
		// TODO handle session specific logic.
		AppUsers appUsers = new AppUsers();
		appUsers.setEmail(user.getEmail());
		appUsers.setFirstName(user.getFName());
		appUsers.setLastName(user.getLName());
		appUsers.setPhoneNumber(user.getPhoneNumber());
		appUsers.setPhoneNumber(passwordEncoder.encode(user.getPassword()));
		//TODO send and email for confirmation
		return ResponseEntity.ok(jwtUserDetailsService.save(appUsers));
	}
	/*
